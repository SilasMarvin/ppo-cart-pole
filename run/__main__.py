...
agent = Model()
optimizer = tf.keras.optimizers.Adam(learning_rate=7e-3, epsilon=1e-5)
...
def actor_loss_func(probs, actions, advs, old_probs):
...
def critic_loss_func(preds, old_preds, returns):
...
def entropy_func(probs, actions):
    return -tf.reduce_sum(probs * tf.math.log(probs + 1e-10))


def train(states, actions, old_preds, advs, old_probs, returns):
    ...
    with tf.GradientTape() as tape:
        p, v = agent(states)
        critic_loss = critic_loss_func(v, old_preds, returns)
        entropy = entropy_func(p, actions)
        actor_loss = actor_loss_func(p, actions, advs, old_probs)
...
for episode in range(EPISODES):
    ...
    if training:
        ...
        values = values[:-1]
        for _ in range(10):
            loss = train(states=states, actions=actions, old_preds=values, advs=advs, old_probs=probs, returns=returns)
